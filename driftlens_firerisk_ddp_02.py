...
Now also:
- Saves best checkpoints (RT-DETR style) per experiment.
- Logs training metrics to TSV.
- Saves DriftLens distances and plots to disk.
"""
...
# ============================================================
# Results helpers (RT-DETR-style outputs)
# ============================================================

RESULTS_ROOT = Path("./results_firerisk")
...
def train_model(model, device, train_loader, val_loader,
                ...
                results_dir: Path = None, exp_name: str = "experiment"):
    """Train model and save RT-DETR-style artifacts:
       - best_checkpoint.pt
       - train_log.tsv
    """
    ...
    log_path = results_dir / "train_log.tsv"
        ...
        if val_acc > best_val_acc:
            ...
            if is_main_process():
                ...
                ckpt = {
                    ...
                    "hparams": {"lr": lr, "weight_decay": wd, "epochs": epochs},
                }
                torch.save(ckpt, results_dir / "best_checkpoint.pt")
...
def run_experiment_new_class(device, data_root, batch_size=64, num_workers=4,
    ...
    _ = train_model(
        model_nc, device, train_loader_nc, val_loader_nc,
        train_sampler=train_sampler_nc, val_sampler=val_sampler_nc,
        epochs=epochs, results_dir=get_results_dir("exp_new_class"),
        exp_name="exp_new_class"
    )
    ...
    results_dir = get_results_dir("exp_new_class")
    np.savez(
        results_dir / "driftlens_new_class.npz",
        distances=distances_nc,
        window_size=window_size,
        approx_drift_window=approx_drift_window
    )
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_dir / "driftlens_new_class.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def run_experiment_blur(device, data_root, batch_size=64, num_workers=4,
    ...
    _ = train_model(
        model_full, device, train_loader_full, val_loader_full,
        train_sampler=train_sampler, val_sampler=val_sampler,
        epochs=epochs, results_dir=get_results_dir("exp_blur"),
        exp_name="exp_blur"
    )
    ...
    results_dir = get_results_dir("exp_blur")
    np.savez(
        results_dir / "driftlens_blur.npz",
        distances=distances_blur,
        window_size=window_size,
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_dir / "driftlens_blur.png", dpi=150, bbox_inches="tight")
    plt.close()
