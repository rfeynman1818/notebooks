# ===============================================================================
# FILE: reports_planned_ct_releases.py
# ===============================================================================
...
# -------------------------------------------------------------------------------
# REPORT: PLANNED CT RELEASES
# -------------------------------------------------------------------------------

def generate_planned_ct_releases_report(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate Planned CT Releases count:
    
    Count records where:
    - ACTL_PCKG_RLS_DATE IS NOT NULL
    - Planned_Unplanned = "Discrete"
    ...
    # Filter for Planned CT (Discrete) releases that have been released
    planned_ct_mask = (
        df["ACTL_PCKG_RLS_DATE"].notna()
        & (df["Planned_Unplanned"] == "Discrete")
    )

    # Aggregate by program and fiscal month
    agg = (
        df.assign(
            planned_ct_cnt=planned_ct_mask.astype(int),
        )
        .groupby(["PRGRM_NAME", "PMM_Program_ID", "Fiscal_Release_Month"], as_index=False)
        .agg(
            planned_ct_releases=("planned_ct_cnt", "sum"),
        )
    )
...
def run_planned_ct_releases_report_from_query(
    cfg: SqlServerConfig,
    *,
    ...
    """
    Run the Planned CT Releases report from a SQL query.
    
    Returns:
        Tuple of (report_df, df_name, csv_path)
    """
    src_df = run_query_df(cfg, sql=sql, params=params, verbose=verbose)
    report_df = generate_planned_ct_releases_report(src_df)
...
def run_planned_ct_releases_report_from_table(
    ...
    *,
    ...
    """
    Run the Planned CT Releases report from a table specification.
