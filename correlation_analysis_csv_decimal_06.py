...
def clean_percentage_columns(df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
    ...
    # Strategy 1: Check columns with '%' or 'pct_' in name
    potential_pct_cols = [col for col in df.columns if '%' in str(col) or 'pct_' in str(col).lower()]
    
    # Strategy 2: Also check ALL object columns for percentage strings
    for col in df.columns:
        if df[col].dtype == 'object' and col not in potential_pct_cols:
