...
# ---- 6.1 Baseline estimation (no per-label PCA) ----
# Empty label list here => only per-batch PCA
...
# ---- 6.2 Threshold estimation (needs a NON-empty label list) ----
# Use the real FireRisk labels for balanced sampling
...
for start in range(0, len(E_stream) - window_size + 1, stride):
    ...
    dist = dl.compute_window_distribution_distances(
        ...
        per_label_sorted=per_label_sorted,  # fine even if per-label PCA is trivial
    )
