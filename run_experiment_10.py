...
# --- 3. Offline Phase: Estimate Baseline and Threshold (FIXED) ---
try:
    print("\n--- 3a. Estimating Baseline ---")
    # FIX: Replaces the failing load_baseline(). This loads the HDF5 data, 
    # computes the internal PCA models, and sets the baseline state.
    ...
    # FIX: Replaces the failing load_threshold(). This uses the internal 
    # baseline state to compute the threshold via K-Fold validation.
    ...
    # Handles errors during the estimation process
...
# This now uses the baseline state set by estimate_baseline()
