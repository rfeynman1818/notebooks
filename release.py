...
# -------------------------------------------------------------------------------
# REPORT: CT RELEASES (ALL)
# -------------------------------------------------------------------------------

def generate_ct_releases_report(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate CT Releases count (all types):
    
    Count records where:
    - ACTL_PCKG_RLS_DATE IS NOT NULL
    
    This counts ALL released CTs regardless of planned/unplanned status.
    ...
    # Filter for all releases that have been released (has a release date)
    ct_releases_mask = df["ACTL_PCKG_RLS_DATE"].notna()

    # Aggregate by program and fiscal month
    agg = (
        df.assign(
            ct_releases_cnt=ct_releases_mask.astype(int),
        )
        .groupby(["PRGRM_NAME", "PMM_Program_ID", "Fiscal_Release_Month"], as_index=False)
        .agg(
            ct_releases=("ct_releases_cnt", "sum"),
        )
    )
...
def run_ct_releases_report_from_query(
    cfg: SqlServerConfig,
    *,
...
def run_ct_releases_report_from_table(
    ...
    *,
    ...
    """
    Run the CT Releases report from a table specification.
