...
# --- 3. Offline Phase: Data Loading and Estimation (Final Key Correction) ---
try:
    print("\n--- 3a. Manually Loading All HDF5 Data ---")
    
    # FINAL KEY ATTEMPT: Revert to standard keys 'embeddings' and 'labels'
    # This assumes the file was created with these names, or the previous logs were misleading.
    
    # 1. Load Baseline Data
    with h5py.File(config["baseline_path"], 'r') as hf:
        # Load the embeddings (E) and labels (Y) arrays
        E_base = hf['embeddings'][:] # Corrected key
        Y_base = hf['labels'][:]     # Corrected key
    print(f"Loaded {len(E_base)} baseline samples.")

    # 2. Load Threshold Data
    with h5py.File(config["threshold_path"], 'r') as hf:
        E_thresh = hf['embeddings'][:] # Corrected key
        Y_thresh = hf['labels'][:]     # Corrected key
