# ===============================================================================
# FILE: reports_preventable_revisions.py
# ===============================================================================
...
# -------------------------------------------------------------------------------
# REPORT: PREVENTABLE REVISIONS
# -------------------------------------------------------------------------------

def generate_preventable_revisions_report(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate Preventable Revisions percentage:
    
    % Preventable = (Count where RSN_FOR_CHG_CODE_MJR = 'Preventable') / 
                    (Count where RSN_FOR_CHG_CODE_MJR in ['Preventable', 'Non-Preventable'])
    
    Grouped by PRGRM_NAME and Fiscal_Release_Month
    """
    required = [
        "CHG_ACTVTY_NUM",
        "RSN_FOR_CHG_CODE_MJR",
    ...
    # Filter for only Preventable or Non-Preventable changes
    valid_mask = df["RSN_FOR_CHG_CODE_MJR"].isin(["Preventable", "Non-Preventable"])
    
    # Identify preventable changes
    preventable_mask = valid_mask & (df["RSN_FOR_CHG_CODE_MJR"] == "Preventable")
    ...
    agg = (
        ...
        .groupby(["PRGRM_NAME", "Fiscal_Release_Month"], as_index=False)
        .agg(
...
def run_preventable_revisions_report_from_query(
    ...
    *,
    ...
    """
    Run the Preventable Revisions report from a SQL query.
    
    Returns:
        Tuple of (report_df, df_name, csv_path)
    """
    src_df = run_query_df(cfg, sql=sql, params=params, verbose=verbose)
    report_df = generate_preventable_revisions_report(src_df)
