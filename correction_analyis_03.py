...
def calculate_correlations(
    ...
    # CRITICAL: Clean any remaining percentage strings before correlation
    # This handles cases where percentage strings weren't caught during loading
    for col in df_metrics.columns:
        if df_metrics[col].dtype == 'object':
            # Check if column contains percentage strings
            sample = df_metrics[col].dropna().head(10)
            if len(sample) > 0 and any('%' in str(val) for val in sample):
                if verbose:
                    print(f"\n⚠️  Found percentage strings in '{col}' - cleaning...")
                # Clean percentage strings: "100%" -> 1.0, "85.5%" -> 0.855
                df_metrics[col] = df_metrics[col].apply(
                    lambda x: float(str(x).strip().rstrip('%')) / 100.0 if pd.notna(x) and '%' in str(x) else x
                )
                # Convert to float
    ...
    if verbose:
        ...
        for col in metric_cols:
            ...
            dtype = df_metrics[col].dtype
            print(f"   - {col}: {n_valid} valid observations (type: {dtype})")
