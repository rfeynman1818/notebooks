...
import sys
import os
import h5py # REQUIRED to manually load data for estimate_baseline
...
# This MUST match the LABEL_MAPPING from your data preparation script.
...
# --- 3. Offline Phase: Data Loading and Estimation (FINAL, TRUE FIX) ---
try:
    print("\n--- 3a. Manually Loading All HDF5 Data ---")
    
    # CRITICAL FIX: Load E and Y for Baseline/Threshold directly into memory (196.png)
    ...
    # 2. Load Threshold Data (Required for KFold_threshold_estimation)
    ...
    # CRITICAL FIX: estimate_baseline requires (E, Y, label_list) as positional arguments.
    ...
    # KFold_threshold_estimation also requires (E, Y) arrays positionally.
    ...
    # Use str(e) to print the content of the ValueError (e.g., the scikit-learn TruncatedSVD error)
    sys.exit(1)
...
# This method still requires the path as it's meant to stream the data from the file.
distances = drift_lens.compute_window_distribution_distances(
    config["data_stream_path"], # 1st Positional: Stream HDF5 Path (The only one that remains a path)
    window_size=config["window_size"],
    batch_n_pc=config["batch_n_pc"],
    per_label_n_pc=config["per_label_n_pc"]
)
