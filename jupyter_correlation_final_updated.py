# ===============================================================================
# JUPYTER NOTEBOOK: CEI/BEI Correlation Analysis
# FINAL VERSION - Decimal Percentages (0-1 scale) + Automatic Cleaning
# ===============================================================================
...
# ==============================================================================
# CELL 3: Load and Validate CEI/BEI Data (UPDATED!)
# ==============================================================================

print("="*80)
print("LOADING AND VALIDATING CEI/BEI DATA")
print("="*80)
...
from correlation_analysis_csv_decimal import (
    ...
    clean_data_quality_issues
)

# 1. LOAD CEI DATA
print("\n" + "="*80)
print("STEP 1: Loading CEI Data")
print("="*80)

cei_df = load_cei_data(
    csv_path="cei_data.csv",  # ← UPDATE THIS PATH
...
# 2. VALIDATE CEI DATA
print("\n" + "="*80)
print("STEP 2: Validating CEI Data Quality")
print("="*80)

cei_quality = generate_data_quality_report(
    cei_df,
...
# 3. LOAD BEI DATA
print("\n" + "="*80)
print("STEP 3: Loading BEI Data")
print("="*80)

bei_df = load_bei_data(
    csv_path="bei_data.csv",  # ← UPDATE THIS PATH
    program_mapping=program_mapping,
    verbose=True
)
...
# 4. VALIDATE BEI DATA
print("\n" + "="*80)
print("STEP 4: Validating BEI Data Quality")
print("="*80)

bei_quality = generate_data_quality_report(
    bei_df,
    check_duplicates_on=['PMM_Program_Name', 'Fm_Reporting_Month'],
    verbose=True
)

# OPTIONAL: Automatically clean data
# Uncomment to enable automatic cleaning:
# print("\n🧹 Cleaning CEI data...")
# cei_df = clean_data_quality_issues(
#     cei_df,
#     remove_duplicates=True,
#     exclude_high_missing_cols=True,
#     missing_threshold=0.5,  # Exclude columns with >50% missing
#     verbose=True
# )
# 
# print("\n🧹 Cleaning BEI data...")
# bei_df = clean_data_quality_issues(
#     bei_df,
#     remove_duplicates=True,
#     exclude_high_missing_cols=True,
...
print("\n✅ CEI and BEI data loaded and validated!")

# ==============================================================================
# CELL 4: Load All Data and Merge
# ==============================================================================

print("="*80)
print("LOADING ALL DATA AND MERGING")
print("="*80)

# NOTE: This cell loads all data fresh and merges everything together.
# If you validated cei_df and bei_df in Cell 3, you can skip this cell
# and use those dataframes directly. Otherwise, run this cell to load everything.
...
complete_data = load_all_data(
    ...
    filter_by_target_dates=True,                 # ← Set to False if date formats don't match
    verbose=True
)
...
# OPTIONAL: Run quality validation on merged data
print("\n" + "="*80)
print("FINAL DATA QUALITY CHECK (Merged Dataset)")
print("="*80)

final_quality = generate_data_quality_report(
    complete_data,
    check_duplicates_on=['PMM_Program_Name', 'Fm_Reporting_Month'],
    verbose=True
)
...
# ==============================================================================
# CELL 5: Data Summary Statistics
# ==============================================================================
...
# ==============================================================================
# CELL 7: Visualize Correlation Matrix
# ==============================================================================
...
# ==============================================================================
# CELL 8: CEI & BEI vs Engineering Metrics
# ==============================================================================
...
# ==============================================================================
# CELL 9: CEI vs BEI Relationship
# ==============================================================================
